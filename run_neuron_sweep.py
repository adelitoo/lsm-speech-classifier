"""
LSM Neuron Parameter Sweep (run_neuron_sweep.py)

This script loops through multiple neuron parameter configurations
(REFRACTORY_PERIOD and MEMBRANE_THRESHOLD) to find the optimal settings for accuracy.

It uses the best topology (K and P) found from the previous sweep and a
fixed, sub-critical weight multiplier.
"""
...
LEAK_COEFFICIENT = 0
WEIGHT_MULTIPLIER = 0.6 # Use the best sub-critical multiplier
SMALL_WORLD_K = 200     # Use best K from previous sweep
SMALL_WORLD_P = 0.1     # Use best P from previous sweep

# --- Neuron Parameter Sweep ---
REFRACTORY_PERIODS_TO_TEST = [2, 4, 6]
MEMBRANE_THRESHOLDS_TO_TEST = [1.5, 2.0, 2.5]
...
def run_experiment_for_neuron_params(refractory, threshold, X_train, y_train, X_test, y_test):


    """


    Runs the full extraction and training pipeline for a *single* set of neuron params.


    """


    # 1. Create a base SimulationParams object for this specific configuration
    ...
    # 2. Calculate w_critico and the final weight
    ...
    print(f"Training and evaluating for Refractory={refractory}, Threshold={threshold}...")
    ...
    return refractory, threshold, avg_activity, test_accuracy
...
if __name__ == "__main__":
    ...
    # 2. Define the parameter combinations to test


    params_to_test = list(itertools.product(REFRACTORY_PERIODS_TO_TEST, MEMBRANE_THRESHOLDS_TO_TEST))





    print(f"\nStarting neuron parameter sweep...")


    print(f"Testing {len(params_to_test)} combinations of (Refractory, Threshold).")


    print(f"Feature set: '{FEATURE_SET_TO_TEST}'")


    print(f"Fixed Topology: K={SMALL_WORLD_K}, P={SMALL_WORLD_P}")
    ...
    results = [] # To store (refractory, threshold, activity, accuracy)
    ...
    for i, (refractory, threshold) in enumerate(params_to_test):


        print("\n" + "="*60)


        print(f"TESTING PARAMS #{i+1}/{len(params_to_test)}: Refractory={refractory}, Threshold={threshold}")
        ...
        r_val, t_val, act, acc = run_experiment_for_neuron_params(


            refractory, threshold, X_train, y_train, X_test, y_test
        ...
        results.append((r_val, t_val, act, acc))


        print(f"✅ Result: Refractory={r_val}, Threshold={t_val}, Activity={act:.1f}%, Accuracy={acc*100:.2f}%")
    ...
    print("NEURON PARAMETER SWEEP FINAL RESULTS")
    ...
    print(f"{'Refractory':<12} | {'Threshold':<11} | {'Activity (%)':<12} | {'Accuracy (%)':<12}")
    ...
    for r, t, act, acc in results:


        marker = "⭐ BEST" if (r == best_result[0] and t == best_result[1]) else ""


        print(f"{r:<12} | {t:<11.2f} | {act:<12.1f} | {acc*100:<12.2f} {marker}")
    ...
    print(f"  Refractory Period = {best_result[0]}")


    print(f"  Membrane Threshold = {best_result[1]:.2f}")
